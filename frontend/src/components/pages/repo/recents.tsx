...
import { getServices, FileService } from "@/services";
...
const Recents = () => {
  ...
  const openRecent = async (path: string) => {
    try {
      setAction("open");
      const dir = await FileService.openRepoDirectory(path);
      console.log({ dir });
    } catch (err) {
      console.log({ err });
...
Recents.loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const recents = await getServices(root).notespace.getRecentNotespaces();
